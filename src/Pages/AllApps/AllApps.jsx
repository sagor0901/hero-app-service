    ...
    <div className="mx-auto w-11/12">
      ...
      <Suspense fallback={<span>Loading</span>}>
        <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {
            fetauredApps.map((app)=><SingleApp key={app.id} app={app}></SingleApp>)
        }
        </div>
      </Suspense>
      <div className="flex justify-center my-5">
        <Link to='/showAllApps' className="btn bg-gradient-to-r from-[#632EE3] to-[#9F62F2] px-12 py-7 text-white text-lg">Show all</Link>
      </div>
    </div>
  );
};
